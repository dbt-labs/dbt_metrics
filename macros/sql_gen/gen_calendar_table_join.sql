...
{% macro default__gen_calendar_table_join(metric_dictionary, calendar_tbl) %}
    ...
    {% if metric_dictionary.window is not none %}
        on cast(calendar_table.{{metric_dictionary.timestamp}} as date) > dateadd({{metric_dictionary.window.period}}, -{{metric_dictionary.window.count}}, calendar_table.date_day)
        and cast(calendar_table.{{metric_dictionary.timestamp}} as date) <= calendar_table.date_day
    {% else %}
        on cast(calendar_table.{{metric_dictionary.timestamp}} as date) = calendar_table.date_day
    {% endif %}
...
{% macro bigquery__gen_calendar_table_join(metric_dictionary, calendar_tbl) %}
    ...
    left join {{calendar_tbl}} calendar_table
    {% if metric_dictionary.window is not none %}
        on cast(calendar_table.{{metric_dictionary.timestamp}} as date) > date_sub(calendar_table.date_day, interval {{metric_dictionary.window.count}} {{metric_dictionary.window.period}})
        and cast(calendar_table.{{metric_dictionary.timestamp}} as date) <= calendar_table.date_day
    {% else %}
        on cast(calendar_table.{{metric_dictionary.timestamp}} as date) = calendar_table.date_day
    {% endif %}
