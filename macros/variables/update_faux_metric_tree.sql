{% macro update_faux_metric_tree(metric_definition, metric_tree, develop_yml, metric_count=999)%}
    ...
    {% for develop_metric_definition in develop_yml.metrics %}
        {# {% set metric_name = develop_metric_definition.name %} #}
        {% do develop_metric_list.append(develop_metric_definition.name ) %}
    {% endfor %}
    {% do print("Develop metric list " ~ develop_metric_list) %}
    ...
    is a leaf node and any recursive loop should end #}
        {% do print("Parent metric list " ~ parent_metrics) %}
        {%- if parent_metrics | length > 0 -%}
            ...
            {%- for parent_metric_name in parent_metrics -%}
                {% do print("Parent metric name " ~ parent_metric_name) %}
                {% do print("Develop metric name in parent_metrics " ~ parent_metric_name in develop_metric_list) %}
                ...
                {% if parent_metric_name in develop_metric_list %}
                    {% do print("This metric is in the develop list " ~ parent_metric_name) %}
                    {% set parent_metric_definition = develop_yml[parent_metric_name] %}
                {% else %}
                    {% do print("We got here with " ~ parent_metric_name) %}
