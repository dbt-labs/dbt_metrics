...
with source_query as (
    ...
    select
        ...
        , {{ metric.type }}( {{ metric.sql }} ) as metric_value
...
with_calcs as (
    ...
    select *
        ...
        */
        ...
        {% for calc in calcs %}
            {% if calc.type == 'period_over_period' and calc.how == 'difference'%}
            , coalesce({{ metric_name }}, 0) - coalesce(
                lag({{ metric_name }}, {{ calc.lag }}) over (
                    partition by {% for dim in dims -%}
            ...
            {% elif calc.type == 'period_over_period' and calc.how == 'ratio'%}
            , coalesce({{ metric_name }}, 0) / nullif(
                lag({{ metric_name }}, {{ calc.lag }}) over (
                    partition by {% for dim in dims -%}
                        {{ dim }} {% if not loop.last %} , {% endif %}
                    {%- endfor %}
            ...
            )::float as calc_{{ loop.index }}

            {% elif calc.type == 'period_over_period' %}
                {% do exceptions.raise_compiler_error("Bad 'how' for period_over_period: " ~ calc.how) %}

            {% elif calc.type == 'rolling' %}
            , {{ calc.aggregate }}({{ metric_name }}) over (
                    partition by {% for dim in dims -%}
                        {{ dim }} {% if not loop.last %} , {% endif %}
                    {%- endfor %}
                    order by period
                    rows between {{ calc.window - 1 }} preceding and current row
            ) as calc_{{ loop.index }}

            {% elif calc.type == 'period_to_date' %}
            , {{ calc.aggregate }}({{ metric_name }}) over (
                    partition by period__{{ calc.period }}
                    {% for dim in dims -%}
                        , {{ dim }}
                    {%- endfor %}
                    order by period
